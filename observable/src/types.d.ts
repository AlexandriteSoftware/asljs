import type { EventMap, Eventful, EventfulFactory as DefaultEventfulFactory } from 'asljs-eventful';
export interface EventfulFactory {
    <T extends object | Function | undefined, E extends EventMap = Record<string | symbol, any[]>>(object?: T): (T extends undefined ? {} : T) & Eventful<E>;
}
/**
...
export interface ObservableOptions {
     ...
     * 'delete', 'define'.
     */
    trace?: ((object: object | Function, action: 'new' | 'set' | 'delete' | 'define', payload?: any) => void) | null;
    /**
...
export type PropString<K> = K extends string ? K : K extends number ? `${K}` : never;
...
export type SetPayload<T> = {
    [K in keyof T]: SetPayloadFor<T, K>;
}[keyof T];
...
} & KeyedSetEvents<T> & KeyedDeleteEvents<T> & KeyedDefineEvents<T>;
...
export type ObservableTraceFn = (object: object | Function, action: 'new' | 'set' | 'delete' | 'define', payload?: any) => void;
export interface ObservableGlobalOptions {
    trace: ObservableTraceFn | null;
}
export type WatchedValues<T, K extends readonly (keyof T)[]> = {
    [I in keyof K]: K[I] extends keyof T ? T[K[I]] : never;
};
export type ObservableWatchFn = {
    <T extends object>(target: T, property: string, callback: (value: any) => void): () => boolean;
    <T extends object, K extends readonly string[]>(target: T, properties: K, callback: (...values: any[]) => void): () => boolean;
};
export type WatchMethod<T extends Eventful> = {
    watch: {
        <K extends Extract<keyof T, string>>(property: K, callback: (value: T[K]) => void): () => boolean;
        <K extends readonly (Extract<keyof T, string>)[]>(properties: K, callback: (...values: WatchedValues<T, K>) => void): () => boolean;
    };
};
export type ObservableArray<T extends readonly any[]> = T & Eventful<ObservableEventsArray<T>> & WatchMethod<T & Eventful<ObservableEventsArray<T>>>;
export type ObservableObject<T extends object> = T & Eventful<ObservableEventsObject<T>> & WatchMethod<T & Eventful<ObservableEventsObject<T>>>;
 ...
 */
export type Observable<T> = T extends readonly any[] ? ObservableArray<T> : T extends object ? ObservableObject<T> : ObservablePrimitive<T>;
export type ObservableEventsArray<T extends readonly any[]> = {
    'set': [ArraySetPayload<T>];
} & {
    'delete': [ArrayDeletePayload<T>];
} & KeyedArraySetEvents<T> & KeyedArrayDeleteEvents<T>;
export type ObservableFn = {
    /** Array overload */
    <T extends readonly any[]>(value: T, options?: ObservableOptions): ObservableArray<T>;
    /** Object overload */
    <T extends object>(value: T, options?: ObservableOptions): ObservableObject<T>;
    /** Primitive overload (boxed as { value }) */
    <T>(value: T, options?: ObservableOptions): ObservablePrimitive<T>;
