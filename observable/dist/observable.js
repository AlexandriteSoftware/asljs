...
import { functionTypeGuard, isFunction, } from './guards.js';
...
const observableImpl = (value, options = {}) => {
    const { eventful: eventfulFn = eventful, trace = null } = options;
    functionTypeGuard(eventfulFn);
    const globalOptions = observable.options;
    const makeProxy = (target, { includeDefine }) => {
        let proxy;
        proxy =
            eventfulFn(new Proxy(target, {
                set(tgt, property, newValue, receiver) {
                    const previous = Reflect.get(tgt, property, receiver);
                    const ok = Reflect.set(tgt, property, newValue, receiver);
                    if (proxy
                        && ok) {
                ...
                defineProperty(tgt, property, descriptor) {
                    if (!includeDefine) {
                        return Reflect.defineProperty(tgt, property, descriptor);
                    }
                    const previous = Object.getOwnPropertyDescriptor(tgt, property) ??
                        null;
                    const ok = Reflect.defineProperty(tgt, property, descriptor);
                    if (proxy
                        && ok) {
                        const payload = {
                            property,
    ...
    if (Array.isArray(value)) {
        const proxy = makeProxy(value, { includeDefine: false });
        if (isFunction(traceFn)) {
            traceFn(proxy, 'new');
        }
        return proxy;
    ...
    if (value !== null
        && typeof value === 'object') {
        const proxy = makeProxy(value, { includeDefine: true });
        if (isFunction(traceFn)) {
