...
`,On="https://api.openai.com/v1/chat/completions",Nn=_(An,"asljs-observable"),Pn=_(kn,"asljs-eventful"),Rn=_(Ln,"asljs-data-binding"),Cn=_(Tn,"asljs-components"),Fn=_(In,"asljs-dali"),Je=Jn();function Mn(){return Je}const Dn=24,Bn=[{type:"function",function:{name:"listFileset",description:"List all file paths in the virtual filesystem.",parameters:{type:"object",properties:{},additionalProperties:!1},strict:!0}},{type:"function",function:{name:"readFile",description:"Read the full text content of a file.",parameters:{type:"object",properties:{path:{type:"string"}},required:["path"],additionalProperties:!1},strict:!0}},{type:"function",function:{name:"setFileContent",description:"Create or fully replace file content.",parameters:{type:"object",properties:{path:{type:"string"},content:{type:"string"}},required:["path","content"],additionalProperties:!1},strict:!0}},{type:"function",function:{name:"replaceFilePart",description:"Replace part of a file by exact search string.",parameters:{type:"object",properties:{path:{type:"string"},search:{type:"string"},replacement:{type:"string"},replaceAll:{type:"boolean"}},required:["path","search","replacement"],additionalProperties:!1},strict:!0}},{type:"function",function:{name:"deleteFile",description:"Delete a file from the virtual filesystem.",parameters:{type:"object",properties:{path:{type:"string"}},required:["path"],additionalProperties:!1},strict:!0}},{type:"function",function:{name:"evalInApp",description:"Evaluate JavaScript in the running app document context.",parameters:{type:"object",properties:{code:{type:"string"}},required:["code"],additionalProperties:!1},strict:!0}}];async function $n(e,n,t){var s,o;const r=[{role:"system",content:Je},{role:"user",content:e}];for(let i=0;i<Dn;i+=1){const f=await fetch(On,{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${n}`},body:JSON.stringify({model:"gpt-4o-mini",temperature:.1,messages:r,tools:Bn,tool_choice:"auto"})});if(!f.ok){const b=await f.json().catch(()=>({})),c=Un(b)??`OpenAI API error: ${f.status}`;throw new Error(c)}const j=(o=(s=(await f.json()).choices)==null?void 0:s[0])==null?void 0:o.message;if(j===void 0)throw new Error("AI returned an unexpected response format.");const m=j.tool_calls??[];if(m.length===0){const b=j.content.trim();return{summary:b===""?"Completed tool-based update.":b}}r.push({role:"assistant",content:j.content,tool_calls:m});for(const b of m){const c=await Vn(b,t);r.push({role:"tool",tool_call_id:b.id,content:c})}}throw new Error("AI exceeded maximum tool steps without completing.")}function Jn(){const e=`
      Latest ASLJS package versions to use:
      - asljs-eventful@${Pn}
      - asljs-observable@${Nn}
      - asljs-data-binding@${Rn}
      - asljs-components@${Cn}
      - asljs-dali@${Fn}
    `,n=[$("eventful",bn,jn),$("observable",vn,wn),$("data-binding",hn,En),$("components",gn,xn),$("dali",yn,Sn)].join(`
...
... [truncated]`}function _(e,n){try{return JSON.parse(e).version??"latest"}catch{return console.warn(`Failed to parse package metadata for ${n}.`),"latest"}}function Un(e){const n=e.error;return typeof(n==null?void 0:n.message)=="string"?n.message:null}async function Vn(e,n){const t=e.function.name,r=_n(e.function.arguments);try{switch(t){case"listFileset":{const s=await n.listFileset();return B(s)}case"readFile":{const s=await n.readFile(O(r,"path"));return B(s)}case"setFileContent":return await n.setFileContent(O(r,"path"),O(r,"content")),B("ok");case"replaceFilePart":return await n.replaceFilePart(O(r,"path"),O(r,"search"),O(r,"replacement"),Gn(r,"replaceAll",!1)),B("ok");case"deleteFile":return await n.deleteFile(O(r,"path")),B("ok");case"evalInApp":{const s=await n.evalInApp(O(r,"code"));return B(s)}default:return Ae(`Unknown tool: ${t}`)}}catch(s){return Ae(s instanceof Error?s.message:String(s))}}function _n(e){try{const n=JSON.parse(e);if(typeof n!="object"||n===null)throw new Error("Tool arguments must be a JSON object.");return n}catch{throw new Error("Invalid tool arguments JSON.")}}function O(e,n){const t=e[n];if(typeof t!="string")throw new Error(`Tool argument "${n}" must be a string.`);return t}function Gn(e,n,t){const r=e[n];if(r===void 0)return t;if(typeof r!="boolean")throw new Error(`Tool argument "${n}" must be a boolean.`);return r}function B(e){return Ue({ok:!0,value:e})}function Ae(e){return Ue({ok:!1,error:e})}function Ue(e){try{return JSON.stringify(e)}catch{return'{"ok":false,"error":"Failed to serialize tool result."}'}}let J=null;const me="asljs-app-builder:eval-request",Ve="asljs-app-builder:eval-response",ke=`<script>
(() => {
  const REQUEST = '${me}';
  const RESPONSE = '${Ve}';
...
<\/script>`;function Wn(e,n){if(J!==null&&(URL.revokeObjectURL(J),J=null),n.length===0){e.src="about:blank";return}const t=n.find(i=>i.name==="index.html")??n.find(i=>i.name.endsWith(".html"))??null;if(t===null){e.src="about:blank";return}let r=t.content;const s=n.find(i=>i.name==="style.css")??n.find(i=>i.name.endsWith(".css"))??null;s!==null&&(r=r.replace(/<link[^>]+href=["']style\.css["'][^>]*>/gi,`<style>${s.content}</style>`),r=r.replace(/<link[^>]+href=["']([^"']+\.css)["'][^>]*>/gi,`<style>${s.content}</style>`));for(const i of n){if(!i.name.endsWith(".js"))continue;const f=i.name.replace(/[.*+?^${}()|[\]\\]/g,"\\$&");r=r.replace(new RegExp(`(<script[^>]*?)\\s+src=["']${f}["']([^>]*)><\\/script>`,"gi"),(y,j,m)=>{const b=`${String(j)} ${String(m)}`;return/type=["']module["']/i.test(b)?`<script type="module">${i.content}<\/script>`:`<script>${i.content}<\/script>`})}r=qn(r);const o=new Blob([r],{type:"text/html"});J=URL.createObjectURL(o),e.src=J}async function Le(e,n){const t=e.contentWindow;if(t===null)throw new Error("Preview frame is not available.");const r=crypto.randomUUID();return new Promise((s,o)=>{const i=window.setTimeout(()=>{y(),o(new Error("Timed out waiting for app evaluation result."))},5e3),f=j=>{if(j.source!==t)return;const m=j.data;if(!(m.type!==Ve||m.id!==r)){if(y(),m.ok===!0){s(m.value);return}o(new Error(m.error??"Unknown preview evaluation error."))}};function y(){window.clearTimeout(i),window.removeEventListener("message",f)}window.addEventListener("message",f),t.postMessage({type:me,id:r,code:n},"*")})}function qn(e){return e.includes(me)?e:e.includes("</body>")?e.replace("</body>",`${ke}</body>`):`${e}
${ke}`}function q(){return crypto.randomUUID()}function C(){return new Date().toISOString()}function p(e){const n=document.getElementById(e);if(n===null)throw new Error(`Missing element #${e}`);return n}const Te=p("app-list"),Ie=p("empty-state"),Oe=p("app-workspace"),Hn=p("app-name-display"),Ne=p("file-tabs"),H=p("file-content"),U=p("chat-messages"),ce=p("chat-input"),de=p("btn-generate"),Kn=p("btn-run"),zn=p("btn-refresh-preview"),K=p("preview-frame"),Yn=p("btn-new-app"),Qn=p("btn-new-app-2"),Xn=p("btn-rename"),Zn=p("btn-delete-app"),et=p("btn-export"),nt=p("btn-settings"),tt=p("btn-agent-instructions"),z=p("settings-modal"),rt=p("btn-close-settings"),st=p("btn-save-settings"),at=p("btn-cancel-settings"),pe=p("api-key-input"),F=p("name-modal"),_e=p("name-modal-title"),N=p("app-name-input"),G=p("btn-confirm-name"),ot=p("btn-cancel-name"),it=p("btn-close-name-modal"),Y=p("import-file"),Q=p("agent-instructions-modal"),ue=p("agent-instructions-text"),lt=p("btn-close-agent-instructions"),ct=p("btn-close-agent-instructions-2"),dt=p("btn-copy-agent-instructions"),Ge="asljs-app-builder-settings";function We(){try{const e=localStorage.getItem(Ge)??"{}";return JSON.parse(e)}catch{return{}}}function pt(e){localStorage.setItem(Ge,JSON.stringify(e))}function qe(){return We().apiKey??""}function ut(){if(a.currentAppId===null)throw new Error("No active app. Create or open an app first.");return a.currentAppId}async function He(){return[...a.files].map(e=>e.name).sort((e,n)=>e.localeCompare(n))}async function ve(e){const n=a.files.find(t=>t.name===e);if(n===void 0)throw new Error(`File not found: ${e}`);return n.content}async function be(e,n){const t=ut(),r=a.files.find(o=>o.name===e);if(r!==void 0){const o={...r,content:n};await le(o),a.files=a.files.map(i=>i.id===o.id?o:i),a.activeFileName=o.name;return}const s={id:q(),appId:t,name:e,content:n};await le(s),a.files=[...a.files,s],a.activeFileName=s.name}async function Ke(e){var r;const n=a.files.find(s=>s.name===e);if(n===void 0)return;await fn(n.id);const t=a.files.filter(s=>s.id!==n.id);a.files=t,a.activeFileName===e&&(a.activeFileName=((r=t[0])==null?void 0:r.name)??null)}async function ze(e,n,t,r=!1){if(n==="")throw new Error("Search text cannot be empty.");const s=await ve(e);if(!s.includes(n))throw new Error(`Search text not found in ${e}.`);let o=s;if(r)o=s.split(n).join(t);else{const i=s.indexOf(n);if(s.indexOf(n,i+n.length)!==-1)throw new Error("Search text is ambiguous. Use replaceAll=true or provide a more specific search block.");o=s.slice(0,i)+t+s.slice(i+n.length)}await be(e,o)}async function Ye(e){if(a.files.length===0)throw new Error("No files available to run.");V();try{return await Le(K,e)}catch{return V(),Le(K,e)}}function Qe(){Te.replaceChildren();const e=[...a.apps].sort((n,t)=>t.updatedAt.localeCompare(n.updatedAt));for(const n of e){const t=document.createElement("div");t.className="app-item"+(n.id===a.currentAppId?" active":""),t.dataset.id=n.id;const r=document.createElement("span");r.className="app-item-name",r.textContent=n.name,t.appendChild(r),t.addEventListener("click",()=>{te(n.id)}),Te.appendChild(t)}}function Xe(){const e=a.apps.find(n=>n.id===a.currentAppId);if(e===void 0){Ie.classList.remove("hidden"),Oe.classList.add("hidden");return}Ie.classList.add("hidden"),Oe.classList.remove("hidden"),Hn.textContent=e.name,he(),ge()}function he(){Ne.replaceChildren();for(const e of a.files){const n=document.createElement("div");n.className="file-tab"+(e.name===a.activeFileName?" active":""),n.textContent=e.name,n.addEventListener("click",()=>{ne(),a.activeFileName=e.name}),Ne.appendChild(n)}}function ge(){const e=a.files.find(n=>n.name===a.activeFileName);H.value=(e==null?void 0:e.content)??"",H.disabled=e===void 0}function Pe(e){a.generating=e,de.disabled=e,de.innerHTML=e?'<span class="spinner"></span> Generating…':"Generate"}function P(e,n){const t=document.createElement("div");t.className=`chat-msg ${e}`;const r=document.createElement("div");r.className="chat-msg-role",r.textContent=e==="user"?"You":"Assistant";const s=document.createElement("div");s.className="chat-bubble",s.textContent=n,t.appendChild(r),t.appendChild(s),U.appendChild(t),U.scrollTop=U.scrollHeight}async function ne(){if(a.activeFileName===null||a.currentAppId===null)return;const e=a.files.find(t=>t.name===a.activeFileName);if(e===void 0)return;const n=H.value;e.content!==n&&(e.content=n,await le(e))}async function te(e){var t;a.currentAppId=e;const n=await un(e);a.files=n,a.activeFileName=((t=n[0])==null?void 0:t.name)??null,U.replaceChildren()}function Ze(){_e.textContent="New App",N.value="",F.classList.remove("hidden"),N.focus(),G.onclick=async()=>{const e=N.value.trim();if(e==="")return;F.classList.add("hidden");const n={id:q(),name:e,createdAt:C(),updatedAt:C()};await ee(n),a.apps=[...a.apps,n],await te(n.id)}}function ye(){F.classList.add("hidden"),G.onclick=null}function ft(){const e=a.apps.find(n=>n.id===a.currentAppId);e!==void 0&&(_e.textContent="Rename App",N.value=e.name,F.classList.remove("hidden"),N.focus(),N.select(),G.onclick=async()=>{const n=N.value.trim();if(n==="")return;F.classList.add("hidden");const t={...e,name:n,updatedAt:C()};await ee(t),a.apps=a.apps.map(r=>r.id===e.id?t:r)})}async function mt(){const e=a.apps.find(n=>n.id===a.currentAppId);e!==void 0&&confirm(`Delete "${e.name}"? This cannot be undone.`)&&(await pn(e.id),a.apps=a.apps.filter(n=>n.id!==e.id),a.currentAppId=null,a.files=[],a.activeFileName=null,U.replaceChildren(),K.src="about:blank")}async function en(){const e=ce.value.trim();if(e==="")return;const n=qe();if(n===""){P("assistant","No OpenAI API key set. Open Settings (⚙) to add your key. Generation is optional — you can also create files manually.");return}if(a.currentAppId===null){P("assistant","Please create or open an app first.");return}ce.value="",P("user",e),Pe(!0);try{const t=await $n(e,n,{listFileset:He,readFile:ve,setFileContent:be,replaceFilePart:ze,deleteFile:Ke,evalInApp:Ye}),r=a.apps.find(s=>s.id===a.currentAppId);if(r!==void 0){const s={...r,updatedAt:C()};await ee(s),a.apps=a.apps.map(o=>o.id===r.id?s:o)}P("assistant",t.summary),V()}catch(t){const r=t instanceof Error?t.message:String(t);P("assistant",`Error: ${r}`)}finally{Pe(!1)}}function V(){ne().then(()=>{Wn(K,a.files)})}async function vt(){const e=a.apps.find(o=>o.id===a.currentAppId);if(e===void 0)return;await ne();const n={app:e,files:a.files,exportedAt:C()},t=new Blob([JSON.stringify(n,null,2)],{type:"application/json"}),r=URL.createObjectURL(t),s=document.createElement("a");s.href=r,s.download=`${e.name.replace(/\s+/g,"-")}.json`,s.click(),URL.revokeObjectURL(r)}function bt(){Y.value="",Y.click()}async function ht(){var n;const e=(n=Y.files)==null?void 0:n[0];if(e!==void 0)try{const t=await e.text(),r=JSON.parse(t);if(r.app===void 0||typeof r.app.name!="string"||!Array.isArray(r.files))throw new Error("Invalid app JSON format.");const s=q(),o={id:s,name:`${r.app.name} (imported)`,createdAt:r.app.createdAt??C(),updatedAt:C()},i=r.files.filter(f=>typeof f.name=="string"&&typeof f.content=="string").map(f=>({id:q(),appId:s,name:f.name,content:f.content}));await ee(o),await mn(s,i),a.apps=[...a.apps,o],await te(s)}catch(t){const r=t instanceof Error?t.message:String(t);alert(`Import failed: ${r}`)}}function gt(){pe.value=qe(),z.classList.remove("hidden"),pe.focus()}function re(){z.classList.add("hidden")}function yt(){const e=We();e.apiKey=pe.value.trim(),pt(e),re()}function wt(){ue.value=Mn(),Q.classList.remove("hidden"),ue.scrollTop=0}function we(){Q.classList.add("hidden")}async function jt(){const e=ue.value;try{await navigator.clipboard.writeText(e),P("assistant","Agent instructions copied to clipboard.")}catch{P("assistant","Could not copy to clipboard automatically. You can still select and copy from the instructions modal.")}}a.on("set:apps",()=>Qe());a.on("set:currentAppId",()=>{Qe(),Xe()});a.on("set:files",()=>{he(),ge()});a.on("set:activeFileName",()=>{he(),ge()});Yn.addEventListener("click",Ze);Qn.addEventListener("click",Ze);Xn.addEventListener("click",ft);Zn.addEventListener("click",()=>{mt()});et.addEventListener("click",()=>{vt()});de.addEventListener("click",()=>{en()});Kn.addEventListener("click",V);zn.addEventListener("click",V);nt.addEventListener("click",gt);tt.addEventListener("click",wt);rt.addEventListener("click",re);st.addEventListener("click",yt);at.addEventListener("click",re);z.addEventListener("click",e=>{e.target===z&&re()});lt.addEventListener("click",we);ct.addEventListener("click",we);dt.addEventListener("click",()=>{jt()});Q.addEventListener("click",e=>{e.target===Q&&we()});G.addEventListener("click",()=>{});ot.addEventListener("click",ye);it.addEventListener("click",ye);F.addEventListener("click",e=>{e.target===F&&ye()});N.addEventListener("keydown",e=>{e.key==="Enter"&&G.click()});ce.addEventListener("keydown",e=>{e.key==="Enter"&&!e.shiftKey&&(e.preventDefault(),en())});H.addEventListener("blur",()=>{ne()});Y.addEventListener("change",()=>{ht()});const se=document.createElement("button");se.className="btn btn-ghost btn-full";se.textContent="↑ Import";se.addEventListener("click",bt);const Re=document.querySelector(".sidebar-footer"),Ce=document.getElementById("btn-settings");Re!==null&&Ce!==null&&Re.insertBefore(se,Ce);window.listFileset=He;window.readFile=ve;window.setFileContent=be;window.replaceFilePart=ze;window.deleteFile=Ke;window.evalInApp=Ye;async function Et(){const e=await dn();if(a.apps=e,e.length>0){const n=[...e].sort((t,r)=>r.updatedAt.localeCompare(t.updatedAt));await te(n[0].id)}else Xe()}Et().catch(e=>{console.error("App Builder init failed:",e)});
