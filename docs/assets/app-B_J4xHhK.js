...
... [truncated]`}function W(e,n){try{return JSON.parse(e).version??"latest"}catch{return console.warn(`Failed to parse package metadata for ${n}.`),"latest"}}function nt(e){const n=e.error;return typeof(n==null?void 0:n.message)=="string"?n.message:null}async function tt(e,n){const t=ze(e),a=st(e.arguments);try{switch(t){case"listFileset":{const s=await n.listFileset();return $(s)}case"readFile":{const s=await n.readFile(N(a,"path"));return $(s)}case"setFileContent":return await n.setFileContent(N(a,"path"),N(a,"content")),$("ok");case"replaceFilePart":return await n.replaceFilePart(N(a,"path"),N(a,"search"),N(a,"replacement"),it(a,"replaceAll",!1)),$("ok");case"deleteFile":return await n.deleteFile(N(a,"path")),$("ok");case"evalInApp":{const s=await n.evalInApp(N(a,"code"));return $(s)}default:return Ne(`Unknown tool: ${t}`)}}catch(s){return Ne(s instanceof Error?s.message:String(s))}}function st(e){if(e===void 0)return{};if(typeof e=="object"&&e!==null)return e;if(typeof e!="string")throw new Error("Invalid tool arguments value.");try{const n=JSON.parse(e);if(typeof n!="object"||n===null)throw new Error("Tool arguments must be a JSON object.");return n}catch{throw new Error("Invalid tool arguments JSON.")}}function at(e){return e.type==="function_call"}function ze(e){if(typeof e.name!="string"||e.name.trim()==="")throw new Error("Tool call missing function name.");return e.name}function rt(e){if(typeof e.call_id!="string"||e.call_id.trim()==="")throw new Error("Tool call missing call_id.");return e.call_id}function ot(e){return typeof e.output_text=="string"&&e.output_text.trim()!==""?e.output_text.trim():Array.isArray(e.output)?e.output.filter(t=>t.type==="message").flatMap(t=>t.content??[]).map(t=>t.text??"").map(t=>t.trim()).filter(t=>t!=="").join(`
`):""}function N(e,n){const t=e[n];if(typeof t!="string")throw new Error(`Tool argument "${n}" must be a string.`);return t}function it(e,n,t){const a=e[n];if(a===void 0)return t;if(typeof a!="boolean")throw new Error(`Tool argument "${n}" must be a boolean.`);return a}function $(e){return Ye({ok:!0,value:e})}function Ne(e){return Ye({ok:!1,error:e})}function Ye(e){try{return JSON.stringify(e)}catch{return'{"ok":false,"error":"Failed to serialize tool result."}'}}const we="asljs-app-builder:eval-request",Qe="asljs-app-builder:eval-response",Pe=`<script>
(() => {
  const REQUEST = '${we}';
  const RESPONSE = '${Qe}';
