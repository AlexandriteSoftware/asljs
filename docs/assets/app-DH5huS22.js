...
`,Hn=[{type:"function",name:"listFileset",description:"List all file paths in the virtual filesystem.",parameters:{type:"object",properties:{},required:[],additionalProperties:!1},strict:!0},{type:"function",name:"readFile",description:"Read the full text content of a file.",parameters:{type:"object",properties:{path:{type:"string"}},required:["path"],additionalProperties:!1},strict:!0},{type:"function",name:"setFileContent",description:"Create or fully replace file content.",parameters:{type:"object",properties:{path:{type:"string"},content:{type:"string"}},required:["path","content"],additionalProperties:!1},strict:!0},{type:"function",name:"replaceFilePart",description:"Replace part of a file by exact search string.",parameters:{type:"object",properties:{path:{type:"string"},search:{type:"string"},replacement:{type:"string"},replaceAll:{type:"boolean"}},required:["path","search","replacement","replaceAll"],additionalProperties:!1},strict:!0},{type:"function",name:"deleteFile",description:"Delete a file from the virtual filesystem.",parameters:{type:"object",properties:{path:{type:"string"}},required:["path"],additionalProperties:!1},strict:!0},{type:"function",name:"evalInApp",description:"Evaluate JavaScript in the running app document context.",parameters:{type:"object",properties:{code:{type:"string"}},required:["code"],additionalProperties:!1},strict:!0}],Kn="https://api.openai.com/v1/responses",Yn=G(Jn,"asljs-observable"),Qn=G(qn,"asljs-eventful"),Xn=G(Wn,"asljs-data-binding"),Zn=G(Gn,"asljs-components"),et=G(zn,"asljs-dali"),Ke=ot(),nt="gpt-5.3-codex",q=20;function tt(){return Ke}const st=12;async function at(e,n,t,a,s){var j;let r,i=e,u=rt(s==null?void 0:s.initialToolStepLimit),g=0;for(;;){if(await U(s,`Step ${g+1}: requesting assistant response...`),g>=u){if(!(await((j=s==null?void 0:s.onToolStepLimit)==null?void 0:j.call(s,{stepsCompleted:g,stepLimit:u}))??!1))throw new Error("AI exceeded maximum tool steps without completing.");u+=st,await U(s,`Extended step limit to ${u}. Continuing...`)}const v=await fetch(Kn,{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${n}`},body:JSON.stringify({model:t,instructions:Ke,temperature:.1,previous_response_id:r,input:i,tools:Hn})});if(!v.ok){const m=await v.json().catch(()=>({})),x=it(m)??`OpenAI API error: ${v.status}`;throw new Error(x)}const y=await v.json();if(!Array.isArray(y.output))throw new Error("AI returned an unexpected response format.");const c=y.output.filter(dt);if(c.length===0){await U(s,`Completed in ${g+1} step(s). Finalizing summary...`);const m=ut(y);return{summary:m===""?"Completed tool-based update.":m}}const b=[];for(const m of c){await U(s,`Step ${g+1}: running ${Ye(m)}...`);const x=await lt(m,a);b.push({type:"function_call_output",call_id:pt(m),output:x})}await U(s,`Step ${g+1}: submitted ${b.length} tool result(s).`),r=typeof y.id=="string"?y.id:r,i=b,g+=1}}function rt(e){if(!Number.isFinite(e))return q;const n=Math.floor(e);return n>=1?n:q}async function U(e,n){(e==null?void 0:e.onProgress)!==void 0&&await Promise.resolve(e.onProgress(n))}function ot(){const e=`
...
... [truncated]`}function G(e,n){try{return JSON.parse(e).version??"latest"}catch{return console.warn(`Failed to parse package metadata for ${n}.`),"latest"}}function it(e){const n=e.error;return typeof(n==null?void 0:n.message)=="string"?n.message:null}async function lt(e,n){const t=Ye(e),a=ct(e.arguments);try{switch(t){case"listFileset":{const s=await n.listFileset();return $(s)}case"readFile":{const s=await n.readFile(N(a,"path"));return $(s)}case"setFileContent":return await n.setFileContent(N(a,"path"),N(a,"content")),$("ok");case"replaceFilePart":return await n.replaceFilePart(N(a,"path"),N(a,"search"),N(a,"replacement"),ft(a,"replaceAll",!1)),$("ok");case"deleteFile":return await n.deleteFile(N(a,"path")),$("ok");case"evalInApp":{const s=await n.evalInApp(N(a,"code"));return $(s)}default:return Pe(`Unknown tool: ${t}`)}}catch(s){return Pe(s instanceof Error?s.message:String(s))}}function ct(e){if(e===void 0)return{};if(typeof e=="object"&&e!==null)return e;if(typeof e!="string")throw new Error("Invalid tool arguments value.");try{const n=JSON.parse(e);if(typeof n!="object"||n===null)throw new Error("Tool arguments must be a JSON object.");return n}catch{throw new Error("Invalid tool arguments JSON.")}}function dt(e){return e.type==="function_call"}function Ye(e){if(typeof e.name!="string"||e.name.trim()==="")throw new Error("Tool call missing function name.");return e.name}function pt(e){if(typeof e.call_id!="string"||e.call_id.trim()==="")throw new Error("Tool call missing call_id.");return e.call_id}function ut(e){return typeof e.output_text=="string"&&e.output_text.trim()!==""?e.output_text.trim():Array.isArray(e.output)?e.output.filter(t=>t.type==="message").flatMap(t=>t.content??[]).map(t=>t.text??"").map(t=>t.trim()).filter(t=>t!=="").join(`
