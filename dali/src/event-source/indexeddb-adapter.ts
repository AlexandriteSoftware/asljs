...
{
  const cursor =
    await dbRequestAsync<IDBCursorWithValue | null>(
...
export class IndexedDbEventSourceAdapter
    ...
    ): Promise<void>
  {
    ...
    if (actualPrevious !== expectedPreviousTransactionId) {
      ...
      throw new EventSourceConflictError(
        `${this.name}: expected previous transaction ${String(expectedPreviousTransactionId)}, actual ${String(actualPrevious)}.`);
    }

    const existing =
      await dbRequestAsync<EventSourceTransaction | undefined>(
        store.get(transaction.id));
