...
test(
  ...
  () => {
    assert.equal(keyValueValid('a'), true);
    assert.equal(keyValueValid(1), true);
    assert.equal(keyValueValid(new Date('2026-03-27T00:00:00.000Z')), true);
    assert.equal(keyValueValid(new Uint8Array([ 1, 2, 3 ])), true);
    assert.equal(keyValueValid([ 'a', 1 ]), true);
  });

test(
  `${TEST_SUITE}: keyValueValid rejects invalid IndexedDB key types`,
  () => {
    assert.equal(keyValueValid(null), false);
    assert.equal(keyValueValid(undefined), false);
    assert.equal(keyValueValid(Number.NaN), false);
    assert.equal(keyValueValid({}), false);
    assert.equal(keyValueValid([ 'a', undefined ]), false);
  });
...
test(
  ...
  () => {
    assert.equal(
      keyValid([ 'id', 'version' ], [ 'a', 'v1' ]),
      true);

    assert.equal(
      keyValid([ 'id', 'version' ], [ 'a' ]),
      false);
