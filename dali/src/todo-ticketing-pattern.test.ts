    ...
    ): Promise<void>
  {
    ...
    if (actualPrevious !== expectedPreviousTransactionId) {
      throw new EventSourceConflictError(
        `${this.name}: head mismatch expected ${String(expectedPreviousTransactionId)} actual ${String(actualPrevious)}.`);
    }
...
test(
  ...
  async () => {
    ...
    assert.equal(sagas[0]?.status, 'completed');
    assert.equal(typeof sagas[0]?.eventTransactionId, 'string');
    ...
    assert.equal(localTransactions.length, 1);
    assert.equal(remoteTransactions.length, 1);
    assert.equal(localTransactions[0]?.id, remoteTransactions[0]?.id);
      ...
      new EventSourceProjectionManager(
        ...
        async transaction => {
          for (const event of transaction.events) {
            ...
            const forward =
              event.forward as { assigneeId?: string; deleted?: string; };
        ...
        () => eventSourceProjectionGet(db, 'ticket-read-model-v1'),
    ...
    assert.equal(beforeRemoteAhead, 0);
    ...
    assert.equal(afterSyncApplied, 1);
    assert.equal(afterRemoteAhead, 1);
  });
