...
test(
  ...
  async () =>
  {
    ...
    await waitFor(
      () => deletedPrevious.length >= 1);

    assert.equal(
      deletedPrevious.length,
      1);
...
test(
  ...
  async () =>
  {
    ...
    await waitFor(
      () => (seen[seen.length - 1] as TestRecord | null)?.value === '20');
