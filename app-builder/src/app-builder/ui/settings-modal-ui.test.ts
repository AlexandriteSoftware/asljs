...
test(
  ...
  async () => {
    const dom = new JSDOM(renderSettingsModal());
    ...
    try {
      ...
      const ui = createSettingsModalUi({
        loadValues: async () => ({
          apiKey: 'sk-demo',
          theme: 'light',
          fontSize: 16,
      ...
      const themeSelect = document.getElementById('theme-select') as HTMLElement & { value: string | null; };
      const fontSizeInput = document.getElementById('font-size-input') as HTMLElement & { value: string | null; };
      const maxToolStepsInput = document.getElementById('max-tool-steps-input') as HTMLElement & { value: string | null; };
      const saveButton = document.getElementById('btn-save-settings') as HTMLElement;

      await ui.open();

      assert.equal(modal.classList.contains('hidden'), false);
      assert.equal(apiKeyInput.value, 'sk-demo');
      assert.equal(themeSelect.value, 'light');
      assert.equal(fontSizeInput.value, '16');
      assert.equal(maxToolStepsInput.value, '30');
...
test(
  ...
  async () => {
    ...
    try {
      ...
      const modal = document.getElementById('settings-modal') as HTMLElement;
      const cancelButton = document.getElementById('btn-cancel-settings') as HTMLElement;
