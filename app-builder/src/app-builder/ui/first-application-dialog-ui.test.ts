...
test(
  ...
  async () => {
    const dom = new JSDOM(renderFirstApplicationDialog());
    ...
    try {
      ...
      const ui = createFirstApplicationDialogUi({
      ...
      const dialog = document.getElementById('first-app-setup') as HTMLElement;
      const nameInput = document.getElementById('first-app-name-input') as HTMLElement & { value: string | null; };
      const apiKeyInput = document.getElementById('first-api-key-input') as HTMLElement & { value: string | null; };
      const createButton = document.getElementById('btn-create-first-app') as HTMLElement;
...
test(
  ...
  async () => {
    ...
    try {
      ...
      const nameInput = document.getElementById('first-app-name-input') as HTMLElement & { value: string | null; focus: () => void; };
      const apiKeyInput = document.getElementById('first-api-key-input') as HTMLElement & { value: string | null; };
      const createButton = document.getElementById('btn-create-first-app') as HTMLElement;
      const sampleButton = document.getElementById('btn-create-todo-sample') as HTMLElement;
      let focused = false;
      ...
      sampleButton.dispatchEvent(new dom.window.MouseEvent('click', { bubbles: true }));
      await flushMicrotasks();

      assert.deepEqual(createdSamples, [
