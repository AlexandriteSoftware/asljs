...
test(
  ...
  () =>
  {
    for (const tool of OPENAI_TOOLS) {
      const propertyKeys =
        Object.keys(
          tool.parameters.properties).sort();
...
test(
  ...
  () =>
  {
    const tool =
      OPENAI_TOOLS.find(
        item => item.name === 'setFilesContent');
...
test(
  ...
  async () =>
  {
    ...
    const output =
      await executeToolCall(
        { type: 'function_call', name: 'not-real-tool' },
        tools);
...
test(
  ...
  async () =>
  {
    ...
    const output =
      await executeToolCall(
        { type: 'function_call', name: 'readFile', arguments: { path: 'x' } },
        tools);
