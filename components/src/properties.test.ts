...
test(
  ...
  async () => {
    ...
    assert.equal(titleInput.theme, theme);
    assert.equal(enabledSelect.theme, theme);
    assert.equal(countInput.theme, theme);
    assert.equal(
    ...
    assert.equal(
      ...
      true);
    assert.equal(titleInput.value, 'Preview');
    assert.equal(enabledSelect.value, 'yes');
    assert.equal(countInput.value, '3');
    assert.equal(itemsInput.value, '[2 items]');
    assert.equal(handlerInput.value, '[Function]');
    assert.equal(optionsInput.value, '[Object]');
    assert.equal(itemsInput.disabled, true);
    assert.equal(handlerInput.disabled, true);
    assert.equal(optionsInput.disabled, true);
  });
...
test(
  ...
  async () => {
    ...
    assert.equal(target.title, 'After');
    assert.equal(target.enabled, true);
    assert.equal(target.count, 42);
  });
  ...
  async () => {
    ...
    element.definition =
      { name: 'LockedDemoModelDefinition',
        properties:
          [ { name: 'title', type: 'string', editable: false },
            { name: 'enabled', type: 'boolean', editable: false } ] };
    element.target = target;
    ...
    assert.equal(enabledSelect.disabled, true);
    ...
    assert.equal(target.title, 'Locked');
...
async function createElement(): Promise<Properties> {
  ...
  return document.createElement('asljs-properties') as Properties;
}
  ...
  ): Promise<void>
{
  ...
  const nestedElements =
    [ ...element.querySelectorAll('asljs-text-input, asljs-select') ] as LitElementLike[];
