    ...
    properties:
      [ { name: 'label',
        ...
        { name: 'autoExtendMaxRows',
          title:
            'Auto extend max rows',
          type: 'number',
          description:
            'Maximum rows when auto extend is enabled.' },
...
export class TextInput extends LitElement
{
  ...
  #model: TextInputTemplateModel = observable(
    { label: '',
      description: '',
      errorMessage: '',
      hideLabel: true,
  ...
  ): MountedTextInputControl
{
  ...
  if (control === null) {
    const fallbackControl =
      multiline
      ? document.createElement('textarea') as
        | HTMLTextAreaElement
        | HTMLInputElement
