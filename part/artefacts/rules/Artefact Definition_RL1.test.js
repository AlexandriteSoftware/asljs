...
{
  const workspacePath = await mkdtemp(path.join(os.tmpdir(), 'part-artefact-definition-rl1-'));
  const articlePath = path.join(workspacePath, 'Article.md');

  await writeFile(articlePath, '# Different Name\n\nNot a definition.\n', 'utf8');
...
test('Artefact Definition_RL1 passes when each declared rule file exists', async () =>
{
  const workspacePath = await mkdtemp(path.join(os.tmpdir(), 'part-artefact-definition-rl1-'));
  const rulesPath = path.join(workspacePath, 'rules');
  const definitionPath = path.join(workspacePath, 'Todo Item.md');

  await mkdir(rulesPath, { recursive: true });
  await writeFile(
    path.join(rulesPath, 'Todo Item_RL1.js'),
...
test('Artefact Definition_RL1 fails when a declared rule file is missing', async () =>
{
  const workspacePath = await mkdtemp(path.join(os.tmpdir(), 'part-artefact-definition-rl1-'));
  const definitionPath = path.join(workspacePath, 'Todo Item.md');
