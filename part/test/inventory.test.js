...
test('inventory respects Definitions parameter and reports Fail when any rule fails', async () =>
{
  ...
  await writeFile(
    path.join(workspacePath, 'definitions', 'Todo Item.md'),
    `# Todo Item
...
`,
  ...
  const stdout = createWritableBuffer();
  const stderr = createWritableBuffer();
  const exitCode = await runCli(['inventory', '--definitions', 'definitions'], {
    cwd: workspacePath,
    stdout,
    stderr,
  });
...
`,
  ...
  const stdout = createWritableBuffer();
  const stderr = createWritableBuffer();
  const exitCode = await runCli(['artefactdefinition', '--definitions', 'definitions'], {
    cwd: workspacePath,
    stdout,
    stderr,
  });
  ...
  assert.equal(exitCode, 0);
  assert.equal(stderr.toString(), '');
  assert.match(stdout.toString(), /\| Name\s+\| Location\s+\|/);
  assert.match(stdout.toString(), /\| Todo Item \| definitions\/Todo Item\.md \|/);
...
`,
  ...
  const stdout = createWritableBuffer();
  const stderr = createWritableBuffer();
  const exitCode = await runCli(['artefactdefinition', 'Requirement'], {
    cwd: workspacePath,
    stdout,
    stderr,
  });
  ...
  assert.equal(exitCode, 0);
  assert.equal(stderr.toString(), '');
  assert.match(stdout.toString(), /- name: Requirement/);
...
test('check prints one row per matched file and rule', async () =>
{
  ...
  await mkdir(path.join(workspacePath, 'artefacts', 'rules'), { recursive: true });
  await mkdir(path.join(workspacePath, 'development', 'features'), { recursive: true });
  await writeFile(
    path.join(workspacePath, 'artefacts', 'Requirement.md'),
    `# Requirement
...
`,
  ...
  await writeFile(
    path.join(workspacePath, 'artefacts', 'rules', 'Requirement_RL11.js'),
    `export async function validate() {
  return;
}
`,
  ...
  await writeFile(
    path.join(workspacePath, 'development', 'features', 'RQ101 Example.md'),
    '# RQ101 Example\n',
    'utf8',
  );
  await writeFile(
    path.join(workspacePath, 'development', 'features', 'RQ102 Example.md'),
    '# RQ102 Example\n',
    'utf8',
  );
  ...
  const stdout = createWritableBuffer();
  const stderr = createWritableBuffer();
  const exitCode = await runCli(['check', 'development/features/*.md', '--with-positives'], {
    cwd: workspacePath,
    stdout,
    stderr,
  });
  ...
  assert.equal(stderr.toString(), '');
  assert.match(stdout.toString(), /\| Path\s+\| Rule\s+\| Result\s+\|/);
  assert.match(stdout.toString(), /\| development\/features\/RQ101 Example\.md \| Requirement_RL10 \| development\/features\/RQ101 Example\.md is not referenced by any test\. \|/);
  assert.match(stdout.toString(), /\| development\/features\/RQ101 Example\.md \| Requirement_RL11 \| OK\s+\|/);
...
`,
  ...
  const stdout = createWritableBuffer();
  const stderr = createWritableBuffer();
  const exitCode = await runCli(['check', 'definitions/Requirement.md', '--with-positives'], {
    cwd: workspacePath,
    stdout,
    stderr,
  });
  ...
  assert.equal(exitCode, 0);
  assert.equal(stderr.toString(), '');
  assert.match(stdout.toString(), /\| definitions\/Requirement\.md \| Article_RL10\s+\| OK\s+\|/);
...
`,
  ...
  await writeFile(
    path.join(workspacePath, 'artefacts', 'Article.md'),
    `# Article
...
`,
  ...
  await writeFile(
    path.join(workspacePath, 'artefacts', 'rules', 'Article_RL10.js'),
    'export async function validate() {}\n',
    'utf8',
  );
  await writeFile(
  ...
  assert.equal(exitCode, 0);
  assert.equal(stderr.toString(), '');
  assert.match(stdout.toString(), /Requirement_RL11/);
  assert.doesNotMatch(stdout.toString(), /Requirement_RL10/);
...
test('check uses artefact locations when pattern is omitted and sorts by path then rule', async () =>
{
  ...
  await mkdir(path.join(workspacePath, 'artefacts', 'rules'), { recursive: true });
  await mkdir(path.join(workspacePath, 'development', 'zeta'), { recursive: true });
  await mkdir(path.join(workspacePath, 'development', 'alpha'), { recursive: true });
  await writeFile(
    path.join(workspacePath, 'artefacts', 'Requirement.md'),
    `# Requirement
...
`,
  ...
  await writeFile(
    path.join(workspacePath, 'development', 'zeta', 'RQ200 Later.md'),
    '# RQ200 Later\n',
    'utf8',
  );
  await writeFile(
    path.join(workspacePath, 'development', 'alpha', 'RQ100 Earlier.md'),
    '# RQ100 Earlier\n',
    'utf8',
  );
  ...
  const stdout = createWritableBuffer();
  const stderr = createWritableBuffer();
  const exitCode = await runCli(['check', '--definitions=Requirement', '--with-positives'], {
    cwd: workspacePath,
    stdout,
    stderr,
  });
  ...
  const rows = stdout.toString()
    .split('\n')
    .filter((line) => line.startsWith('| development/'))
...
`,
  ...
  await writeFile(path.join(workspacePath, 'artefacts', 'rules', 'Requirement_RL10.js'), 'export async function validate() { throw new Error(\'Failed.\'); }\n', 'utf8');
  await writeFile(path.join(workspacePath, 'artefacts', 'rules', 'Requirement_RL11.js'), 'export async function validate() {}\n', 'utf8');
  await writeFile(path.join(workspacePath, 'development', 'RQ101 Example.md'), '# RQ101 Example\n', 'utf8');
  ...
  assert.equal(exitCode, 1);
  assert.equal(stderr.toString(), '');
  assert.match(stdout.toString(), /Requirement_RL10/);
...
`,
  ...
  const stdout = createWritableBuffer();
  const stderr = createWritableBuffer();
  const exitCode = await runCli(['check', '--with-positives'], {
    cwd: workspacePath,
    stdout,
    stderr,
  });
  ...
  assert.match(stdout.toString(), /Requirement_RL10/);
  assert.match(stdout.toString(), /Requirement_RL11/);
  assert.match(stdout.toString(), /\| development\/RQ101 Example\.md \| Requirement_RL11 \| OK\s+\|/);
});
...
test('cli reports missing option values as command errors', async () =>
{
  ...
  const missingValueExitCode = await runCli(['inventory', '--definitions'], {
    cwd: process.cwd(),
    stdout,
    stderr,
  });
