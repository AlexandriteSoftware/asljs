...
import { toTypeId } from './markdown.js';
...
export class Definition
{
  ...
  static async load(filePath, options = {})
  {
    ...
    return Definition.parse(content, {
      ...options,
      filePath,
    });
  }
  ...
  static async parse(content, options = {})
  {
    ...
    const description = extractDescription(content, tree.children, heading);
    const locationSection = getSection(tree.children, content, 'Location');

    if (!locationSection) {
      return null;
    }

    const location = parseLocation(locationSection.raw, locationSection.nodes);

    if (!location) {
      return null;
    }

    const propertiesSection = getSection(tree.children, content, 'Properties');
    const propertyEntries = parseProperties(propertiesSection?.nodes ?? []);
    const propertyDefinitions = new Map(propertyEntries.map((entry) => [entry.normalizedLabel, entry.propertyName]));
    const rulesSection = getSection(tree.children, content, 'Rules');
    const ruleEntries = parseRules(rulesSection?.nodes ?? []);
    const rules = await loadRules(ruleEntries, {
    ...
    return new Definition({
      ...
      description,
      location,
      properties: Object.fromEntries(propertyEntries.map((entry) => [entry.propertyName, entry.description])),
      rules,
      propertyDefinitions,
...
function getFirstHeading(nodes)
{
  return nodes.find((node) => node.type === 'heading' && node.depth === 1) ?? null;
}

function extractDescription(content, nodes, heading)
{
...
function getSection(nodes, content, sectionName)
{
  const headingIndex = nodes.findIndex((node) => node.type === 'heading' && node.depth === 2 && extractText(node) === sectionName);
  ...
  return {
    nodes: sectionNodes,
    raw: sliceNodes(content, sectionNodes),
  };
}

function parseLocation(rawSection, nodes)
{
  const listItems = getListItemTexts(nodes);
  ...
  if (listItems.length > 0) {
    ...
    for (const itemText of listItems) {
      const typeMatch = itemText.match(/^(Files|Folders)\s*:\s*(.+)$/i);

      if (typeMatch) {
        location.type = normalizeLocationType(typeMatch[1]);
        location.pattern = typeMatch[2].trim();
        continue;
      }

      const excludeMatch = itemText.match(/^Exclude\s*:\s*(.+)$/i);

      if (excludeMatch) {
        location.exclude.push(excludeMatch[1].trim());
        continue;
      }
  ...
  const inlineCodeMatch = rawSection.match(/`([^`]+)`/);
...
function parseProperties(nodes)
{
  return getListItemTexts(nodes)
    .map((itemText) => itemText.match(/^(.*?)\s*(?::|-)\s+(.+)$/))
...
{
  return getListItemTexts(nodes)
    .map((itemText) => itemText.match(/^([A-Z]+\d+)\s*(?:-\s*)?(.+)$/))
    .filter((match) => match !== null)
    .map((match) => ({
      id: match[1],
      description: match[2].trim(),
...
async function loadRules(ruleEntries, options)
{
  return Promise.all(ruleEntries.map(async (ruleEntry) => {
    const resolvedRuleFile = await resolveRuleFile(ruleEntry.id, options);
...
async function resolveRuleFile(ruleId, options)
{
  ...
  const candidateDirectories = [
    path.join(path.dirname(options.definitionPath), 'rules'),
    path.join(options.rootPath, 'part'),
  ];
  ...
  for (const directoryPath of candidateDirectories) {
    ...
    for (const entry of entries) {
      ...
      return {
        absoluteFilePath,
        filePath: toPosixPath(path.relative(path.dirname(options.definitionPath), absoluteFilePath)),
      };
    }
  }
...
function sliceNodes(content, nodes)
{
  if (nodes.length === 0) {
    return '';
  }
...
function extractText(node)
{
  ...
  if (Array.isArray(node.children)) {
    return node.children.map((childNode) => extractText(childNode)).join('');
  }
