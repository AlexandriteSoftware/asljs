...
test('DefinitionProvider returns definition markdown files and excludes gitignored files', async () =>
{
  ...
  await mkdir(path.join(workspacePath, 'hidden'), { recursive: true });
  await writeFile(path.join(workspacePath, '.gitignore'), 'hidden/\n', 'utf8');
  await writeFile(
...
`,
  ...
  await writeFile(
    path.join(workspacePath, 'hidden', 'Hidden Item.md'),
    `# Hidden Item
...
`,
  ...
  const provider = new DefinitionProvider(workspacePath);
  const definitions = await provider.getDefinitions();

  assert.equal(definitions.length, 1);
  assert.equal(definitions[0].name, 'Todo Item');
});