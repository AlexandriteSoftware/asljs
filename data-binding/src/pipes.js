...
const DATE_STYLE_NAMES = new Set(['short', 'medium', 'long', 'full']);
/**
 ...
 * ```
 */
export function createBuiltInPipes(locale) {
    return {
        string: value => {
            if (value === null
                || value === undefined) {
        ...
        date: (value, format = 'short') => {
            ...
            return formatDateOrIntl(value, format, locale, false);
        },
        datetime: (value, format = 'short') => {
            if (value === null
                || value === undefined) {
                return value;
            }
            return formatDateOrIntl(value, format, locale, true);
        },
        fixed: (value, digitsText = '2') => {
            if (value === null
                || value === undefined) {
                return value;
            }
        ...
        json: (value, spacesText = '0') => {
            if (value === null
                || value === undefined) {
                return value;
            }
            const spaces = Number(spacesText);
            const formatted = JSON.stringify(value, null, Number.isInteger(spaces) && spaces >= 0
                ? spaces
                : 0);
            return formatted ?? '';
        },
        default: (value, ...fallbackParts) => {
            if (value === null
                || value === undefined) {
                return value;
            }
 ...
 * ```
 */
export function mergePipes(options) {
    const builtIns = createBuiltInPipes();
    return {
        ...builtIns,
        ...(options?.pipes ?? {})
    };
}
function formatDateOrIntl(value, format, locale, withTime) {
